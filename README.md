...
| FFN 中间维 | 3456 | 5632 |
| 训练 token | 25B（≈50 tok/param） | 25B（≈23 tok/param） |
| 8×H100 竞价耗时 | **~6 小时 ≈ $71** | **~13 小时 ≈ $157** |
...
**为什么主线选 500M 而不是 1B**（同样 25B tokens 的预算下）：

1. **同样 25B tokens，500M 吃得饱、1B 吃不饱**。500M 拿到 50 tokens/param（Chinchilla 最优的 2.5 倍），1B 只有 23（刚到最优）。SmolLM2、Qwen2.5 这些实际好用的小模型走的都是 over-train 路线——**推理成本导向下，Chinchilla 最优并不是最优**。
...
**全项目约 $135**（P0–P8，全部按 8×H100 竞价 $12/小时计），其中正式预训练只占 $71。
...
**算力买的是模型质量，不是学习收益。** 分词器、数据管线、架构实现、数值对齐、训练框架、缩放律、消融——这些占了项目的主体，加起来不到 $50。P6 只是把一条已验证的流水线放大跑一次。

免费路径：Google TPU Research Cloud 申请通过可免费用 TPU v4 一个月（需迁到 JAX）。明细见 [docs/04-compute.md](docs/04-compute.md)。

...
> 数据量只有 9.44B tokens，**下载和处理的工作量比原计划小一个数量级**：FineWeb-Edu 和 CCI3-HQ 各取一小部分分片即可。
>
> **两份数据，放在不同地方**：
>
> | 用途 | 规模 | 存放 |
> |---|---|---|
> | P5 全部实验（缩放律、seed 方差、消融） | 2B tokens，4 GB | Rangpur home |
> | P6 正式训练 | ≥10.4B tokens，~21 GB | 对象存储（Rangpur home 只有 16 GB） |
>
> 两份用**同一个分词器、同一个配比**，P5 那份是 P6 那份的子集。处理都在 Rangpur `cpu` 分区完成：原始数据下载到计算节点本地盘 `$TMPDIR`（~197 GB），分词后写回 home 或逐分片上传。详见 [09-rangpur.md](09-rangpur.md) §三、§4.4。
...
> **P2-1 实际做法**（`src/mytransformer/data/filters.py`，校准见 [learn/p2-1-filters.md](learn/p2-1-filters.md)）：
> 选用的来源都已经由原作者做过语言识别和质量分类器过滤，所以 2.1 的语言识别和 2.3 的分类器**不再重复做**，
> 中文用"汉字占比 ≥ 40%"兜底。2.2 的 Gopher 规则照 datatrove 的实现，中文按字算、n 乘以 3；
> 代码用 StarCoder 规则；数学、百科只做长度和重复度检查。
...
| 单进程处理 | 慢 100 倍 | `multiprocessing` 按 shard 并行；Rangpur 上按分片拆成多个 `cpu` 作业 |
| HF 缓存落进 home | 瞬间撑爆 16 GB 配额 | `streaming=True`，或 `HF_HOME=$TMPDIR/hf` |

...
- [x] 12 个实验跑完（2026-09-27，实际 17.7 A100 小时）：`reports/ablation.md`（总表 + 分子集表，各子集用自己的种子噪声）、`reports/scaling.md`
- [x] 种子 σ = 0.0041；6 组消融全部超过 2σ，但 A1、A5 混着学习率、A6 同时改了两处、A4 是中英取舍（详见报告 §四）
- [ ] 缩放律外推不稳定（2.27–2.94）：先跑 `ladder_s1_smallbatch` 诊断实验
- [ ] 据此做的决定：待确认（报告 §五）；下游任务分推迟到 P8

...
| 事项 | 怎么确认 |
|---|---|
| ~~A100 节点是否也有本地 `$TMPDIR`~~ | ✅ 有（冒烟 600172，节点 `a100-a`）：`/scratch/<作业号>`，200 GB 盘，当时剩 140 GB——**与同节点上的其他作业共用** |
| `$TMPDIR` 在作业结束后是否被清空 | 按"会清空"来设计，不依赖它 |
| `a100-test` 是否也需要 `--account=comp3710` | 加上能正常排队运行（冒烟 600172）；不加会怎样未测 |
| ~~真实 `SIGUSR1` 为什么没生效~~ | ✅ 已解决（冒烟 600202 通过）：Slurm **没有**屏蔽它（`/proc` 显示批处理 shell 的掩码里没有 `SIGUSR1`）；是 `torch.compile` 第一次运行时，某个底层库在 C 层替换了处理函数。训练循环在第一步后重新安装处理函数即可。具体是哪个库未查明 |
